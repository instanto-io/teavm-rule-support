...
abstract class TestEntryPointTransformer implements ClassHolderTransformer, TeaVMPlugin {
    ...
    private void generateApplyRulesProgram(MethodHolder method, ClassHierarchy hierarchy) {
        ...
        for (RuleEntry rule : rules) {
            if (rule.ruleType == TYPE_METHOD_RULE) {
                frameworkMethod = pe.invoke(TestEntryPoint.class.getName(), "frameworkMethod",
                        ValueType.object(FRAMEWORK_METHOD), testCaseVar, name);
                break;
            }
        }
        ...
        for (RuleEntry rule : rules) {
            ...
            statement = rule.ruleType == TYPE_TEST_RULE
                    ? ruleValue.cast(ValueType.object(TEST_RULE))
                            .invokeVirtual("apply", ValueType.object(STATEMENT), statement,
                                    description)
                    : ruleValue.cast(ValueType.object(METHOD_RULE))
                            .invokeVirtual("apply", ValueType.object(STATEMENT), statement,
                                    frameworkMethod, testCaseVar);
        }
     ...
     *
     * <p>A rule implementing both interfaces is applied once, as a {@code TestRule}; that mirrors
     * the dedup in {@code BlockJUnit4ClassRunner.withRules}. Returns -1 when the member is not a
     * rule at all.
     */
    ...
    private void generateAddLaunchersWithProvider(MethodReader testMethodReader, ClassHierarchy hierarchy,
        ...
        if (providerMethod == null) {
            throw new IllegalStateException("No TestNG data provider '" + providerName
                    + "' found in " + testMethodReader.getOwnerName()
                    + " for " + testMethodReader.getName());
        }
