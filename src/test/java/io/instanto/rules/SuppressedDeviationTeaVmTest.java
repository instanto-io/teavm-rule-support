 ...
 * are split: the source constructors carry {@code @Rename("fakeInit")} and are never invoked, while
 * ordinary methods carry {@code @Rename("<init>")} and become the real constructors. Java compiles
 * field initialisers into constructors, so the array is initialised in the discarded half and every
 * throwable is left with a null {@code suppressed}. {@code suppressionEnabled} is set in the
 * surviving half, so {@code addSuppressed} passes its guard and then dereferences null.
 *
...
public class SuppressedDeviationTeaVmTest {
  ...
  public void getSuppressedStillThrowsOnThisTeaVm() {
    ...
    assertThrows(
        "TeaVM now initialises suppressed - the workaround can be removed",
